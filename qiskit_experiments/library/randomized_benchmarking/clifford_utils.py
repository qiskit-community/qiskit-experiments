...
def _clifford_1_qubit_instruction(
    num: Integral, basis_gates: Optional[Tuple[str]] = None
) -> Instruction:
    return clifford_1_qubit_circuit(num, basis_gates).to_instruction()


@lru_cache(maxsize=2048)
def _clifford_2_qubit_instruction(
    num: Integral, basis_gates: Optional[Tuple[str]] = None
) -> Instruction:
    return clifford_2_qubit_circuit(num, basis_gates).to_instruction()
...
def clifford_1_qubit_circuit(
    ...
    if basis_gates:
        qc = transpile(qc, basis_gates=list(basis_gates))
...
def _unpack_num(num, sig):
    r"""Returns a tuple :math:`(a_1, \ldots, a_n)` where :math:`0 \le a_i \le \sigma_i` where
    sig=:math:`(\sigma_1, \ldots, \sigma_n)` and num is the sequential number of the tuple
    """
    res = []
    for k in sig:
        res.append(num % k)
...
def _unpack_num_multi_sigs(num, sigs):
    """Returns the result of `_unpack_num` on one of the signatures in `sigs`"""
