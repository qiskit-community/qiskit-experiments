...
class CurveAnalysis(BaseAnalysis, ABC):
    ...
    def _generate_fit_guesses(self, **extra_options) -> Union[Dict[str, Any], List[Dict[str, Any]]]:
        ...
        2. :meth:`_finalize_fit_options` applies user provided options for ``p0`` and ``bounds``.

        3. :meth:`_reduce_duplicated_fit` removes duplicated configurations.
    ...
    def _finalize_fit_options(self, **fitter_options) -> Dict[str, Any]:
        """Finalize algorithmically generated fitting options.

        This method performs following things:

        - Extract user provided initial guesses and bounds from fit options. Then,
          format the options to a dictionary with a key of fit parameters,
          then override fitting options with the values if they are not ``None``.

        - Check if the dictionary key matches with the parameter names defined in the
          ``self.__series__``. If parameter value is missing, substitute default values.

        Args:
            fitter_options: Fit options generated by `self._generate_fit_guesses`.
    ...
    def _reduce_duplicated_fit(fit_candidates: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        If multiple fit configurations are provided by :meth:`_generate_fit_guesses` but
    ...
    def _run_analysis(
        ...
        try:
            ...
            fit_candidates = self._generate_fit_guesses(**extra_options)
            ...
            fit_options_candidates = self._reduce_duplicated_fit(
                [self._finalize_fit_options(**fit_options) for fit_options in fit_candidates]
            )
            fit_results = []
