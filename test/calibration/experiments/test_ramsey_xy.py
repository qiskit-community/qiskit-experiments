...
class TestRamseyXY(QiskitTestCase):
    ...
    def test_update_calibrations(self):
        ...
        cals = BackendCalibrations(FakeArmonk(), library)

        tol = 1e4  # 10 kHz resolution

        # Check qubit frequency before running the cal
        f01 = cals.get_parameter_value("qubit_lo_freq", 0)
        self.assertTrue(len(cals.parameters_table(parameters=["qubit_lo_freq"])["data"]), 1)
        ...
        FrequencyCal(cals, 0, backend, osc_freq=osc_shift).run().block_for_results()

        # Check that qubit frequency after running the cal is shifted by freq_shift, i.e. 4 MHz.
        f01 = cals.get_parameter_value("qubit_lo_freq", 0)
        self.assertTrue(len(cals.parameters_table(parameters=["qubit_lo_freq"])["data"]), 2)
