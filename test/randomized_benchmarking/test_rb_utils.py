...
class TestRBUtilities(QiskitExperimentsTestCase):
    ...
    def test_number_to_clifford_mapping_1q(self):
        ...
        transpiled_circs_file = (
            root_dir
            + "/../../qiskit_experiments/library/randomized_benchmarking/transpiled_circs_1q_rz_sx.qpy"
        )
        ...
        for index, qc in enumerate(all_transpiled_circuits):
            num = CliffordUtils.compose_num_with_clifford(
                num_qubits=1, composed_num=0, qc=qc, basis_gates=transpiled_cliff_names
            )
            assert num == index
    ...
    def test_number_to_clifford_mapping_2q(self):
        ...
        transpiled_circs_file = (
            root_dir
            + "/../../qiskit_experiments/library/randomized_benchmarking/transpiled_circs_2q_rz_sx_cx.qpy"
        )
        if os.path.isfile(transpiled_circs_file):
            with open(transpiled_circs_file, "rb") as fd:
                all_transpiled_circuits = qpy.load(fd)
        ...
        for index, qc in enumerate(all_transpiled_circuits):
            ...
            num = CliffordUtils.compose_num_with_clifford(
                num_qubits=2, composed_num=0, qc=qc, basis_gates=transpiled_cliff_names
            )
            assert num == index
