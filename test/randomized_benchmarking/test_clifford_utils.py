...
class TestCliffordUtils(QiskitExperimentsTestCase):
    ...
    def test_number_to_clifford_mapping_single_gate(self):
        ...
        for inst in transpiled_cliff_list:
            num = CliffordUtils.num_from_clifford_single_gate(
                inst, qubits, rb_num_qubits=1, basis_gates=transpiled_cliff_names
            )
        ...
        for inst in general_cliff_list:
            num = CliffordUtils.num_from_clifford_single_gate(
                inst, qubits, rb_num_qubits=1, basis_gates=general_cliff_names
            )
            qc_from_num = CliffordUtils.clifford_1_qubit_circuit(num=num)
            qr = QuantumRegister(1)
            qc_from_inst = QuantumCircuit(qr)
    ...
    def test_number_to_clifford_mapping_1q(self):
        ...
        """
        transpiled_circs_file = CliffordUtils.file_name(num_qubits=1, basis_gates=self.basis_gates)
        ...
        for index, qc in enumerate(all_transpiled_circuits):
            num = CliffordUtils.compose_num_with_clifford(
                num_qubits=1, composed_num=0, qc=qc, basis_gates=self.basis_gates
            )
            assert num == index
    ...
    def test_number_to_clifford_mapping_2q(self):
        ...
        corresponds to the index of the circuit qc.
        """
        transpiled_circs_file = CliffordUtils.file_name(num_qubits=2, basis_gates=self.basis_gates)
        if os.path.isfile(transpiled_circs_file):
            with open(transpiled_circs_file, "rb") as fd:
                all_transpiled_circuits = qpy.load(fd)
        else:
        ...
        for index, qc in enumerate(all_transpiled_circuits):
            ...
            num = CliffordUtils.compose_num_with_clifford(
                num_qubits=2, composed_num=0, qc=qc, basis_gates=self.basis_gates
            )
            assert num == index
    ...
    def test_compose_by_num_1q(self):
        ...
        for _ in range(num_tests):
            ...
            clifford_from_num = CliffordUtils.clifford_1_qubit_circuit(result_by_num)
