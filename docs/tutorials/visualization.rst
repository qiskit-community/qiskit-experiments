...
.. jupyter-execute::
    ...
    t1_exp = T1(physical_qubits=(0, ), delays=delays, backend=backend)

    t1_data = t1_exp.run().block_for_results()
    t1_data.figure(0)
...
.. jupyter-execute::
    ...
    plotter = t1_exp.analysis.plotter
...
.. jupyter-execute::
    ...
    t1_exp.analysis.set_options(plot_residuals=True)

    # Re-run analysis
    t1_data_res = t1_exp.analysis.run(t1_data).block_for_results()
    t1_data_res.figure(0)
...
Now we specify the figure options before running the analysis for a second time:
...
.. jupyter-execute::
    ...
    exp_data2 = exp.analysis.run(exp_data).block_for_results()
    exp_data2.figure(0)
...
options. The following block uses the previous T1 experiment to provide examples
of options that have not been demonstrated until now in this tutorial:

.. jupyter-execute::

   t1_exp.analysis.set_options(plot_residuals=False)
   plotter = t1_exp.analysis.plotter
